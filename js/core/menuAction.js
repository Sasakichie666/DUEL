...
// 修改：新增墓场/放逐区检索功能，修复 modifyStats 步长不生效问题
...
(function(global) {
    ...
    function searchDeck(deckType) {
        ...
        const title = deckType === 'mainDeck' ? '主牌库检索' : (deckType === 'costDeck' ? '费用牌库检索' : '传说牌库检索');
        if (global.ViewerUI) {
    ...
    function modifyStats(card, anchorEl) {
        ...
        if (!card.cardData || card.cardData.type !== 'minion') {
            if (global.UI && global.UI.showNotification) {
                global.UI.showNotification('只有随从卡可以修改攻防', 'warning');
            }
            return;
        }

        // 移除旧的自定义菜单
        const existing = document.querySelector('.custom-modify-menu');
        if (existing) existing.remove();

        const menu = document.createElement('div');
        menu.className = 'context-menu custom-modify-menu';

        const btnContainer = document.createElement('div');
        btnContainer.className = 'modify-buttons';

        const createActionBtn = (label, action) => {
            const btn = document.createElement('button');
            btn.className = 'context-menu-item modify-action-btn';
        ...
        // 修复：按钮点击时动态获取当前步长
        const attackUpBtn = createActionBtn(`攻 +${global.StatManager?.getStep() || 1}`, () => {
            const step = global.StatManager?.getStep() || 1;
            global.StatManager?.modifyAttack(card, step);
        });
        const attackDownBtn = createActionBtn(`攻 -${global.StatManager?.getStep() || 1}`, () => {
            const step = global.StatManager?.getStep() || 1;
            global.StatManager?.modifyAttack(card, -step);
        });
        const defenseUpBtn = createActionBtn(`防 +${global.StatManager?.getStep() || 1}`, () => {
            const step = global.StatManager?.getStep() || 1;
            global.StatManager?.modifyDefense(card, step);
        });
        const defenseDownBtn = createActionBtn(`防 -${global.StatManager?.getStep() || 1}`, () => {
            const step = global.StatManager?.getStep() || 1;
            global.StatManager?.modifyDefense(card, -step);
        });

        btnContainer.appendChild(attackUpBtn);
        btnContainer.appendChild(attackDownBtn);
        btnContainer.appendChild(defenseUpBtn);
        btnContainer.appendChild(defenseDownBtn);

        const stepContainer = document.createElement('div');
        stepContainer.className = 'step-selector';
        ...
        steps.forEach(value => {
            ...
            btn.dataset.step = value;
            if (value === (global.StatManager?.getStep() || 1)) btn.classList.add('active');
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                global.StatManager?.setStep(value);
                stepButtons.forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                const currentStep = global.StatManager?.getStep() || 1;
                attackUpBtn.textContent = `攻 +${currentStep}`;
                attackDownBtn.textContent = `攻 -${currentStep}`;
                defenseUpBtn.textContent = `防 +${currentStep}`;
                defenseDownBtn.textContent = `防 -${currentStep}`;
            });
        ...
        menu.appendChild(stepContainer);
    ...
    function positionMenu(menuEl, anchorEl) {
        ...
        if (left + menuRect.width > viewportWidth) left = anchorRect.left - menuRect.width - 8;
