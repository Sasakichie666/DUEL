...
(function(global) {
    ...
    function modifyStats(card, anchorEl) {
        ...
        const step = global.StatManager ? global.StatManager.getStep() : 1;
        ...
        const attackUpBtn = createActionBtn(`攻 +${step}`, () => global.StatManager?.modifyAttack(card, step));
        const attackDownBtn = createActionBtn(`攻 -${step}`, () => global.StatManager?.modifyAttack(card, -step));
        const defenseUpBtn = createActionBtn(`防 +${step}`, () => global.StatManager?.modifyDefense(card, step));
        const defenseDownBtn = createActionBtn(`防 -${step}`, () => global.StatManager?.modifyDefense(card, -step));
        ...
        steps.forEach(value => {
            ...
            if (value === step) btn.classList.add('active');
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
